...
Display requested address derived by given [BIP44 path](./path) on device and returns it to caller. User is presented with a description of the requested key and asked to confirm the export on OneKey.
...
- `path` - _required_ `string | Array<number>` minimum length is `5`. [read more](./path)
